...
import {
  ...
  X, Paperclip, Tag, User, Calendar, Hash, Layers,
} from 'lucide-react';
...
// stats = { progress, total, completed, inProgress, pending }
// progress is computed from the project's tasks, not a schema field
      ...
      >
        ...
        >
          ...
          <div className="flex items-start justify-between p-5 border-b border-slate-100 dark:border-slate-800">
            ...
            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex-shrink-0 ml-2">
              <X size={15} />
            </button>
          </div>
            ...
            {task.tags?.length > 0 && (
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {task.tags.map((t) => (
                    <span key={t} className="text-[11px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500">{t}</span>
                  ))}
                </div>
              </div>
            )}
                  ...
                  {task.attachments.map((att, i) => {
                    ...
                    return (
                      <a key={i} href={url} target="_blank" rel="noreferrer"
                        className="flex items-center gap-2.5 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                        <Paperclip size={12} className="text-slate-400 flex-shrink-0" />
                        <span className="text-[12px] text-indigo-600 dark:text-indigo-400 truncate flex-1">{att.name || att.url}</span>
                      </a>
                    );
                  })}
                </div>
        ...
        >
          ...
          <div className="overflow-y-auto p-5 space-y-4">
            ...
            {todo.description && (
              <div>
                <p className="text-[10.5px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Description</p>
                <p className="text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-wrap">{todo.description}</p>
              </div>
            )}
...
export default function ClientPortalPage() {
  ...
  // Project schema has no `progress` field — it must be derived
    ...
    <Page>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {tasks.length > 0 && (() => {
            // Two metrics: % tasks marked completed + avg task-level progress
            ...
            return (
              <div className="card p-5">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[15px] font-bold text-indigo-500">{completionPct}%</span>
                </div>
                ...
                {/* Task-level avg progress (how far along each task individually) */}
                {taskStats.avgProgress > 0 && taskStats.avgProgress !== completionPct && (
                  <>
                    <div className="flex items-center justify-between mt-3 mb-1">
                      ...
                      <span className="text-[11px] font-semibold text-slate-500">{taskStats.avgProgress}%</span>
                    </div>
                    <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
          ...
          <div className="card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-[13.5px] font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                  ...
                  return (
                    <div key={status}>
                      <div className="px-4 py-1.5 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                        <span className={`badge ${cfg?.tw || 'badge-neutral'} text-[10px]`}>{cfg?.label || status}</span>
                        <span className="text-[10px] text-slate-400 ml-2">{group.length}</span>
                      </div>
            ...
            <div className="space-y-2">
              {Object.entries(PRIORITY_CONFIG).map(([k, v]) => (
                <div key={k} className="flex items-center gap-2">
                  ...
                  <span className="text-[12px] text-slate-600 dark:text-slate-400 capitalize">{k}</span>
                </div>
              ))}
            </div>
            <p className="text-[10.5px] text-slate-400 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
